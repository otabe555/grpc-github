...
import (
	...
	model "github.com/otabe555/grpc/model"
...
func (cm *ContactManager) GetAllContacts() ([]model.Contact, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&user.ID, &user.Name, &user.Phone, &user.Gender, &user.Email, &user.CreatedAt)
		if err != nil {
			return nil, err
		}
...
func (cm *ContactManager) GetContact(id int) (*model.Contact, error) {
	...
	err := row.Scan(&user.ID, &user.Name, &user.Phone, &user.Gender, &user.Email, &user.CreatedAt)
